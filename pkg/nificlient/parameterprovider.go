...
func (n *nifiClient) CreateParameterProvider(entity nigoapi.ParameterProviderEntity) (*nigoapi.ParameterProviderEntity, error) {
	...
	ppEntity, rsp, body, err := client.ControllerApi.CreateParameterProvider(context, entity)
...
func (n *nifiClient) UpdateParameterProvider(entity nigoapi.ParameterProviderEntity) (*nigoapi.ParameterProviderEntity, error) {
	// Get nigoapi client, favoring the one associated to the coordinator node.
	client, context := n.privilegeCoordinatorClient()
	if client == nil {
		n.log.Error("Error during creating node client", zap.Error(ErrNoNodeClientsAvailable))
	...
	ppEntity, rsp, body, err := client.ParameterProvidersApi.UpdateParameterProvider(context, entity.Id, entity)
